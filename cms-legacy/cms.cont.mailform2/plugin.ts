...
async function render(node: Node): Promise<HtmlString> {
  ...
  for (const [id, field] of Object.entries(elements)) {
    ...
    const options = (await node.showText(id + "_value")).plain().split(";").map((value) => value.trim()).filter(Boolean);
    ...
    rows.push(html`<tr><td><label>${label.html()}${field?.obl ? "*" : ""}</label></td><td>${input}</td></tr>`);
  }
