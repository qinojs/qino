...
export default async function (el, { node, signal }) {
  ...
  el.head = t`SEO`;
  el.badge = descr.value?.trim() ? '' : '!'; // same data as the body — no second round-trip

  await el.html`<div class=-seo>
    ${t`Title`}:
  ...
  el.on('input', '.-desc', (d) => { check(d); el.badge = d.value.trim() ? '' : '!'; });
  check(el.querySelector('.-desc'));
}
