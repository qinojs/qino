...
export const suspiciousPaths = (app: App, signal: AbortSignal): void => report(app, signal, 15, "suspicious path", {
...
}, async () => await app.settings.security.foreignPaths ?? true);

// An empty list becomes (?!), which never matches.
const any = (list: string[] = []) => list.map(RegExp.escape).join("|") || "(?!)";

/** Reports a 404 whose path matches one of the lists (case-insensitive); a path that exists or is redirected costs nothing. */
function report(app: App, signal: AbortSignal, weight: number, reason: string, lists: Lists, enabled = async (): Promise<unknown> => true): void {
