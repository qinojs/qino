...
export default async function codemirrorView(file: string): Promise<string> {
  ...
  <div style="height:100%;width:100%">
    <textarea id=editor name="textareaContentCanBeCachedOnReload${Date.now()}" mime="${hee(mime)}" line="${hee(line)}" col="${hee(col)}" style="width:100%;height:100%">${hee(content)}</textarea>
  </div>`;
}
