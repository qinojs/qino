...
async function render(node: Node): Promise<string> {
  ...
  const recent = await db.all(
    ...
    "  ) y" +
    ") x WHERE x.rn=1 ORDER BY x.time DESC LIMIT 20",
  ).catch(() => []);
  let recentRows = "";
...
<div class=u2-card>
  ...
  const top = await db.all(
    "SELECT page_id, COUNT(*) AS n FROM _vers_page_text WHERE _vers_log > 0 GROUP BY page_id ORDER BY n DESC LIMIT 20",
  ).catch(() => []);
  let topRows = "";
  for (const r of top) {
...
<div class=u2-card>
  ...
  const changed = await db.all(
    ...
    "WHERE d.space <> 0 AND d.changed_page > l.changed_page",
  ).catch(() => []);
  let changedRows = "";
