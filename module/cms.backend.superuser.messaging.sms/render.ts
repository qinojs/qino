...
export async function send(node: Node): Promise<HtmlString> {
  ...
  const [groupRows, userRows] = await Promise.all([
    db.query`
      ...
      WHERE p.verified IS NOT NULL AND (p.main = ${true} OR NOT EXISTS (
...
export async function phones(node: Node): Promise<HtmlString> {
  const t = node.app.t;
  const rows = await phoneList(node.app);
  const labels = {
  ...
  const body = rows.length
    ? html.join(rows.map((p) => phone(p, labels)))
    : html`<tr><td colspan=7>${await t`No phone numbers yet.`}`;
...
function phone(p: Row, labels: Record<string, string>): HtmlString {
  return html`<tr>
    ...
    <td>${p.number}
    <td>${p.main ? "✓" : ""}
    <td>${p.verified ? u2.time(p.verified) : html`<span class=u2-badge>${labels.pending}</span>`}
    <td>${p.error ? html`<span class=u2-badge title="${p.error}">${String(p.error).slice(0, 24)}</span>` : ""}
    <td>${u2.time(p.created)}
    <td>
      ${p.verified ? "" : html`<button type=button class=u2-unstyle data-approve="${p.id}" title="${labels.approve}">✓</button>`}
      ${p.verified && !p.main ? html`<button type=button class=u2-unstyle data-main="${p.id}" title="${labels.main}">★</button>` : ""}
      ${p.verified ? html`<button type=button class=u2-unstyle data-test="${p.id}" title="${labels.test}"><u2-ico icon=send>➤</u2-ico></button>` : ""}
