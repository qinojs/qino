...
Publishes plain text and reads `channel_post`, `edited_channel_post`, group messages and replies
from the existing Telegram webhook. Bots cannot fetch history, so posts are mirrored from the moment
the webhook is active; `sync()` fetches nothing here.
