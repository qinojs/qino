...
// Seconds the first paid try costs; every further one doubles it. Longer than retyping a password
// takes, or the wait is always over before the next attempt arrives and nobody ever meets it.
const STEP = 5;
const MAX = 300;
...
export async function proofWait(app: App, usrId: number): Promise<number> {
  ...
  if (unixTime() - last > FORGET) return 0;
  return Math.max(0, last + wait(Number(row.fails)) - unixTime());
}
