...
function open(el){
    el.dispatchEvent(new CustomEvent('c1-dialog-open',{bubbles:true,cancelable:true}));
    setTimeout(() => backdrop.for(el));
}
function close(el){
    backdrop.hide();
    el.dispatchEvent(new CustomEvent('c1-dialog-close',{bubbles:true,cancelable:true}));
}
...
bdDiv.style.cssText = 'position:fixed; top:-70px; left:0; bottom:0; right:0; background:rgba(0,0,0,.4); transition:opacity .16s linear; opacity:0';
