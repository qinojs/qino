...
import { ReqUrl } from "../lib/ctx/ReqUrl.ts";

const url = new ReqUrl("http://user@qino.test:8080/a%2Fb/c?x=1&x=2#frag");

Deno.test("ReqUrl: exposes the read-only URL parts", () => {
...
Deno.test("ReqUrl: instance is frozen and has no searchParams backdoor", () => {
  assert(Object.isFrozen(url));
  assertEquals("searchParams" in url, false);
  assert(!(url instanceof URL));
});

Deno.test("ReqUrl: toURL() returns an independent mutable native URL", () => {
...
Deno.test("ReqUrl: stringifies to href", () => {
