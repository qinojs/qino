cms.initCont('cms.backend.users', el=>{
    ...
    if (list) {
		...
		el.c1Find('.-new-email').addEventListener('input', async function(e){
			const used = await $fn('page::api')(pid, {email_used:this.value});
			const message = used ? 'Diese E-Mail-Adresse wird bereits verwendet' : '';
