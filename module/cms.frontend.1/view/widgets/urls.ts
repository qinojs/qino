...
export default async function (node: Node): Promise<string> {
  ...
  for (const row of urlRows) {
    urlTrs += `<tr data-lang="${row.lang}">
      ...
      <td style="width:10px"><input class=-custom type=checkbox ${row.custom ? "checked" : ""} title="${hee(await app.t`benutzerdefiniert`)}">
      <td style="width:10px"><input class=-target ${row.target ? "checked" : ""} type=checkbox title="${hee(await app.t`Neues Fenster`)}">`;
  }
  ...
  let redirectTrs = `<tr>
    ...
    <td style="width:100px"><button class=-add>${await app.t`hinzufügen`}</button>`;
  for (const row of redirectRows) {
    redirectTrs += `<tr itemid="${hee(row.request)}">
      <td style="width:20px;white-space:nowrap;padding-right:0"><small style="font-size:.7em">${host}${ctx.appURL}</small>
      <td style="padding-left:5px">${hee(row.request)}
      <td class=-delete style="cursor:pointer;width:20px"><img src="${ctx.sysURL}cms.frontend.1/pub/img/delete.svg" alt="${hee(await app.t`löschen`)}">`;
  }

  return `<div class=qgCmsFront1UrlManager pid=${node}>
  <table class="-urls -styled -noborder" style="width:100%"><tbody>${urlTrs}</table>
  <br>
  <b>${await app.t`Direktlinks`}</b>
