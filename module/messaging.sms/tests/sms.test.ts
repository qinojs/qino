...
const makeApp = (db: Db) => ({
  db,
  t,
  settings: { "messaging.sms": { _secret: "test-secret", provider: {} } },
...
Deno.test("a user can verify multiple phones but cannot claim another user's number", async () => {
  ...
  assert(!("verify_hash" in first));
  assertEquals(messages.length, 1);

  await assertRejects(() => verifyPhone(app, 1, Number(first.id), "000000"), ApiError, "Verification code is invalid");
  const verified = await verifyPhone(app, 1, Number(first.id), codeFrom(messages[0].text));
  assert(verified.verified);
  assertEquals(Boolean(verified.main), true);

  const pending = await addPhone(app, 1, "+41791234568");
  assertEquals((await userPhones(app, 1)).length, 2);
  await assertRejects(() => addPhone(app, 2, "+41791234567"), ApiError, "Phone number is unavailable");
  await assertRejects(() => addPhone(app, 2, String(pending.number)), ApiError, "Phone number is unavailable");
  await db.table("usr_phone").update(pending.id, { verify_expires: 1, verify_sent: 1 });
  assertEquals((await addPhone(app, 2, String(pending.number))).usr_id, 2);
});
...
Deno.test("one verified phone becomes main and users can switch or delete the main number", async () => {
  ...
  const first = await addPhone(app, 1, "+41791234567");
  await verifyPhone(app, 1, Number(first.id), codes[String(first.number)]);
  const second = await addPhone(app, 1, "+41791234568");
  await verifyPhone(app, 1, Number(second.id), codes[String(second.number)]);
...
Deno.test("trusted administration can approve a phone without its code", async () => {
  ...
  const pending = await addPhone(app, 1, "+41791234567");
  const approved = await approvePhone(app, Number(pending.id));
...
Deno.test("verification resends are limited and five wrong attempts invalidate the code", async () => {
  ...
  const phone = await addPhone(app, 1, "+41791234567");

  await assertRejects(() => addPhone(app, 1, "+41791234567"), ApiError, "Wait before requesting");
  for (let i = 0; i < 5; i++) {
    await assertRejects(() => verifyPhone(app, 1, Number(phone.id), "000000"), ApiError);
  }
  await assertRejects(() => verifyPhone(app, 1, Number(phone.id), codeFrom(messages[0])), ApiError, "expired");
});
...
Deno.test("send reaches only verified phones selected by user, group or all", async () => {
  ...
  const first = await addPhone(app, 1, "+41791234567");
  await verifyPhone(app, 1, Number(first.id), codes[String(first.number)]);
  const second = await addPhone(app, 2, "+41791234568");
  await verifyPhone(app, 2, Number(second.id), codes[String(second.number)]);
