...
import { ApiError, beforeProof, identified, proofFailed, unixTime } from "@qino/qino";
...
export async function verify(ctx: Ctx, code: string): Promise<boolean> {
  ...
  await beforeProof(ctx.app, usrId); // six digits are guessable; the wait is what makes them not
