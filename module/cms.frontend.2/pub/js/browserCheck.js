function run() {
  ...
  let html =
        '<dialog style="background:#fff; padding:20px; box-shadow:0 0 8px rgba(0,0,0,.5)" class=qgCMS>'+
  ...
  dlg.querySelector('.-closeBtn').addEventListener('click', () => {
    const url = new URL(location.href);
    url.searchParams.set('cms_editmode', '0');
    location.href = url;
  });
  dlg.querySelector('.-ignoreBtn').addEventListener('click', () => {
    document.cookie = 'qcms_browserCheck_ignore=1; max-age=300';
    dlg.remove();
  });

  document.body.append(dlg);
  dlg.showModal(); // top layer, no z-index needed
}
