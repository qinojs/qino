...
const panelRoot = cms.panelRoot || document;
const treeContainer = panelRoot.getElementById("tree");
const pageAddInp = panelRoot.getElementById("page-add");
const showContents = () => cms.panel.state.has("tree_show_c")?.get({ silent: true });

  // var dblClick = false;
  cms.Tree = $(treeContainer).dynatree({
    debugLevel: 0,
    activeVisible: true,
    onActivate(node) {
      cms.Tree?.onActivate?.(node); // engine-neutraler Hook (panel.mjs)
      if (node.data.myaccess < 2 || node.data.type === "c") {
        $(pageAddInp).css({ opacity: .2 }).attr("disabled", true);
      } else {
        $(pageAddInp).css({ opacity: 1 }).attr("disabled", false);
      }
