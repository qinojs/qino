...
function logRefColumns(db: App["db"]): { table: string; col: string }[] {
  ...
  const tables = (db.schema?.properties ?? {})<string, { additionalProperties?: { properties?: Record<string, unknown> } }>;
...
<tr u2-href>
    ...
    <td>${row.usr_id ? `<span style="color:${uniqueColor(row.usr_id)}">${hee(
  (row.firstname ?? "") + " " + (row.lastname ?? "")
)}</span><br><small>${hee(row.email)}</small>` : "<small>guest</small>"}
