...
const known = new WeakMap<App, { source: string; account: Promise<Account> }>();

async function config(app: App): Promise<{ base: URL; token: string; source: string }> {
  ...
  return { base, token, source: `${base.origin}\0${token}` };
}
...
async function account(app: App): Promise<Account> {
  const cfg = await config(app);
  let cached = known.get(app);
  if (cached?.source === cfg.source) return cached.account;
  cached = { source: cfg.source, account: call(app, "/api/v1/accounts/verify_credentials") };
  cached.account.catch(() => known.delete(app));
  known.set(app, cached);
  return cached.account;
}

const targetId = (base: URL, account: Account) => `${base.origin}#${account.id}`;
...
export const socialProvider: Provider = {
  ...
  async publish(app, target, text, key) {
    if (!text) throw new ProviderError("social.mastodon: text is empty");
    const [cfg, user] = await Promise.all([config(app), account(app)]);
    if (target !== targetId(cfg.base, user)) throw new ProviderError(`social.mastodon: unknown target ${target}`);
